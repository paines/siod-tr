/* 
   name:    gd.c
   purpose: interface to libgd 2.3.3+ https://libgd.github.io/
   author:  george j. carrette (original 1996)
...
{char version[256];
 sprintf(version, "libgd 2.3.3+ bindings (modernized 2025, original: $Id: gd.c,v 1.2 1996/11/04 15:23:37 gjc Exp $)");
...
/* Note: If you hack your gd library source like I did you can add a function that
   ...
   Updated to use mkstemp() instead of mktemp() for security (2025)
*/
...
{FILE *f;
 ...
 /* mkstemp() creates AND opens the file atomically - much safer than mktemp() */
 fd = mkstemp(tmpname);
 if (fd == -1) {
     *len = 0;
     return;
 ...
 /* Convert file descriptor to FILE* for use with gdImageGif */
 f = fdopen(fd, "wb+");
 if (f != NULL) {
     gdImageGif(im, f);
     fstat(fd, &sb);
     ...
     rewind(f);
     bytes_read = fread(buffer, 1, *len, f);
     *len = bytes_read; /* Update len with actual bytes read */
     fclose(f); /* This also closes fd */
 } else {
     close(fd); /* fdopen failed, close the fd manually */
     *len = 0;
 }
 unlink(tmpname);
}
...
{gdImagePtr im;
 ...
 color = get_c_long(poparg(&l,NIL));
 gdImageString(im,font,x,y,(unsigned char *)c,color);
 return(NIL);}
...
{gdImagePtr im;
 ...
 c = get_c_string(poparg(&l,NIL));
 color = get_c_long(poparg(&l,NIL));
 gdImageStringUp(im,font,x,y,(unsigned char *)c,color);
 return(NIL);}
...
{long j;
 ...
 /* Use modern gdFontGet* functions instead of extern declarations */
 setvar(cintern("gdFontGiant"),
	lcgdFontCreate(gdFontGetGiant()),
	NIL);
 setvar(cintern("gdFontLarge"),
	lcgdFontCreate(gdFontGetLarge()),
	NIL);
 setvar(cintern("gdFontMediumBold"),
	lcgdFontCreate(gdFontGetMediumBold()),
	NIL);
 setvar(cintern("gdFontSmall"),
	lcgdFontCreate(gdFontGetSmall()),
	NIL);
 setvar(cintern("gdFontTiny"),
	lcgdFontCreate(gdFontGetTiny()),
	NIL);
